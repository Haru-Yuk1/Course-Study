...
![image-20231207112738087](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231207112738087.png)
...
![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/20210227112230368.png)

[其他有关概念](http://t.csdnimg.cn/rVrJt)
...
<img src="%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231219155105178.png" alt="image-20231219155105178" style="zoom:150%;" />

下图是一个无向图和其邻接矩阵：

![202103011006555](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/202103011006555.png)

下图是有向图和它的邻接矩阵：

![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JlYWxfRm9vbF8=,size_16,color_FFFFFF,t_70-17028707596763.png)

<img src="%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231219155142253.png" alt="image-20231219155142253" style="zoom: 150%;" />

#### 二、邻接表

**邻接表是指对图G中的每个顶点$v_i$建立一个单链表，第i个单链表中的结点表示依附于顶点$v_i$的边(对于有向图则是以顶点$v_i$为尾的弧)，这个单链表就称为顶点$v_i$的边表(对于有向图则称为出边表)**。边表的头指针和顶点的数据信息采用顺序存储(称为顶点表)，所以在邻接表中存在两种结点:顶点表结点和边表结点，如下图所示。

![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/20210301165106350.png)
...
![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JlYWxfRm9vbF8=,size_16,color_FFFFFF,t_70-17030595963935.png)
...
适用范围：要求==有向图，且有入度为0的节点（则也有出度为0的节点），且没有环==

**操作：**从入度为0的点出发，去掉这个点并去掉它的边，将它排到第一个，则会出现新的入度为0的点，将它和它的边去掉，并排到后面，如图

<img src="%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231220165617003.png" alt="image-20231220165617003" style="zoom: 50%;" />

<img src="%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231220165629922.png" alt="image-20231220165629922" style="zoom:50%;" />

<img src="%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231220165701737.png" alt="image-20231220165701737" style="zoom:50%;" />
...
<font color='red'>注意：P算法和K算法都是针对无向图的算法，主要都是考虑形不形成环</font>

![image-20231222151424158](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231222151424158.png)
...
![image-20231220201723485](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231220201723485.png)
...
unordered_set<Edge*> primMST(Graph graph){
    ...
    for(auto node:graph.nodes){                 //遍历图的节点
...
![image-20231220202113294](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231220202113294.png)
...
<font color='red'>不用并查集时，注意set的初始化，在for循环内初始化是会使地址一直一样，需要使用new操作符来动态分配内存</font>
...
class MySets{
    public:
        ...
        MySets(list<Node*> nodes){
            ...
            for(auto cur :nodes){
                set=new list<Node*>();  //必须使用new操作符来动态分配内存，不然每个循环初始时地址相同
                set->push_back(cur);
...
![image-20231222151458618](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231222151458618.png)

#### 1. 迪杰斯特拉( Dijkstra )算法

==Dijkstra算法用于构建单源点的最短路径==，所以也被称为**单源最短路径算法**，即图中某个点到任何其他点的距离都是最短的。例如，构建地图应用时查找自己的坐标离某个地标的最短距离。可以用于有向图，但是不能存在负权值。Dijkstra算法是一个贪心算法
...
##### <font size='4'>1. 二叉排序树的查找</font>

  假设我们要查找的值为value，我们从根结点开始，比较结点的值和value的大小：

- 如果结点的值等于value，返回

- 如果结点的值比value小，那么在左子树查找

- 如果结点的值比value大，那么在右子树查找
...
![img](https://img-blog.csdnimg.cn/20210309100019953.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JlYWxfRm9vbF8=,size_16,color_FFFFFF,t_70#pic_center)
...
![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JlYWxfRm9vbF8=,size_16,color_FFFFFF,t_70-17028912471813.png)

##### 2.平衡二叉树的查找

平衡二叉树的查找直接调用二叉排序树的查找即可

##### 3.平衡二叉树的插入和删除

二叉排序树保证平衡的**基本思想**如下：==每当在二叉排序树中插入(或删除)一个结点时，首先检查其插入路径上的结点是否因为此次操作而导致了不平衡。==若导致了不平衡，则先找到插入路径上离插入结点最近的平衡因子的绝对值大于1的结点A，再对以A为根的子树，在保持二叉排序树特性的前提下，调整各结点的位置关系，使之重新达到平衡。
**注意:每次调整的对象都是最小不平衡子树，即以插入路径上离插入结点最近的平衡因子的绝对值大于1的结点作为根的子树。下图中的虚线框内为最小不平衡子树。**

![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JlYWxfRm9vbF8=,size_16,color_FFFFFF,t_70-17028917359526.png)



平衡二叉树的插入过程的前半部分与二叉排序树相同，但在新结点插入后，若造成查找路径上的某个结点不再平衡，则需要做出相应的调整。可将调整的规律归纳为下列4种情况：

1. **LL平衡旋转(右单旋转)**。<font color='red'>由于在结点A的左孩子(L)的左子树(L)上插入了新结点</font>，A的平衡因子由1增至2，导致以A为根的子树失去平衡，需要一次<font color='red'>向右</font>的旋转操作。将A的左孩子B向右上旋转代替A成为根结点，将A结点向右下旋转成为B的右子树的根结点，而B的原右子树则作为A结点的左子树。

如下图所示，结点旁的数值代表结点的平衡因子，而用方块表示相应结点的子树，下方数值代表该子树的高度。

![img](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/20210309101721760.png)

2. **RR平衡旋转(左单旋转)**。类1.

![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/2021030910190515.png)

3. **LR平衡旋转(先左后右双旋转)**。由于在A的左孩子的右子树上插入新节点，A的平衡因子由1增至2，导致以A为根的子树失去平衡，需要进行两次旋转操作，先左旋转后右旋转。

![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JlYWxfRm9vbF8=,size_16,color_FFFFFF,t_70-170289262930113.png)

4. **RL平衡旋转(先右后左双旋转)**。

![在这里插入图片描述](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L1JlYWxfRm9vbF8=,size_16,color_FFFFFF,t_70-170289265566016.png)
...
3. 散列函数==应尽量简单，能够在较短==的时间内计算出任一关键字对应的散列地址。

下面介绍常用的散列函数。

##### 1、直接定址法

直接取关键字的某个==线性函数值==为散列地址，散列函数为
$$
...
![image-20231219144205682](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231219144205682.png)
...
![image-20231219144318829](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231219144318829.png)

##### 4、折叠法

![image-20231219145419526](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231219145419526.png)

##### 5、除留余数法

这是一种==最简单、最常用==的方法，假定散列表表长为m，取一个不大于m但最接近或等于m的质数p，利用以下公式把关键字转换成散列地址。散列函数为
$$
H(key)=key\%p(p\le m)
$$
...
H(key)= random(key)
$$
其中random为随机函数。==当关键字长度不等时，采用这个方法构造散列函数比较合适。==
...
H_i(key)=(H(key)+d_i)\%m
$$
式中，H(key)为散列函数；$i=0,1,2,\dots,k(k\le m-1)$ m为散列表长

![image-20231219145911590](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231219145911590.png)
...
![image-20231219152538976](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231219152538976.png)
...
有三个可以使用的杆，将从大到小摆放的圆盘移到另一个杆子上。圆盘在的杆子为from，圆盘要去的杆子为to，剩下的杆子叫other
...
### 二、打印一个字符串的全部子序列（子集），包括空字符串

**解决方法：**

- 遍历所有的字符，每个字符都有两种处理方式，要或者不要
- 处理要和不要两种状态，如果要，将字符加到要输出的list中，如果不要，就不加字符
- 遍历到最后一个字符，将其输出

![image-20231225101526345](%E7%AE%97%E6%B3%95%E8%AF%BE%E7%A8%8B%E6%80%BB%E7%BB%93.assets/image-20231225101526345.png)
...
### 三、打印一个字符串的全部排列，要求不要出现重复的排列.

- 全排列是指，相同的字符，将字符放在不同顺序的排列。
- 每个位置尝试放字符串里的所有字符。



### 四、给定一个整型数组arr，代表数值不同的纸牌排成一条线。
